...
def strip_accents_keep_letters(text: str) -> str:
    #Replace accented characters with non-accented equivalents where possible.
    normalized = unicodedata.normalize("NFKD", text)
    return "".join(ch for ch in normalized if not unicodedata.combining(ch))

def basic_reddit_clean(text: str) -> str:
    ...
    # Normalize accents
    text = strip_accents_keep_letters(text)
...
def neutralize_gender_self_id(text: str, token: str = "gender") -> str:
...
def keep_punctuation_as_tokens(text: str) -> str: # Convert the chosen punctuation into alphabetic tokens
    if not isinstance(text, str):
        return ""
...
def load_clean_and_filter_once(
    csv_path: str,
    ...
    df["post_basic"] = df["post_raw"].apply(basic_reddit_clean)
    df["post_basic"] = df["post_basic"].apply(neutralize_gender_self_id)
...
def preprocess_gender_dataset(
    ...
    if df_preloaded is None:
        df = load_clean_and_filter_once(csv_path, allowed_languages=allowed_languages)
    else:
        df = df_preloaded.copy()

    if keep_punct_tokens:
        df["post_basic"] = df["post_basic"].apply(keep_punctuation_as_tokens)
...
if __name__ == "__main__":
    ...
    base_df = load_clean_and_filter_once(DATA_PATH, allowed_languages={"en"})

    # 1) First dataset (compelte cleaning)
    df_clean = preprocess_gender_dataset(
        csv_path=DATA_PATH,
    ...
    df_clean_punct = preprocess_gender_dataset(
        csv_path=DATA_PATH,
        allowed_languages={"en"},
        spacy_model="en_core_web_sm",
        min_token_len=2,
